...
public interface ICommand {
        ...
        for (i = 0; i < args.length;
            ...
            try {
                ...
                if ((memoryEntry = Variable.getEntryByName(varName)) != null) {
                    ...
                    args[i] = toSplit.trim();
                    System.out.println("replaced var");
                }
            } catch (Exception e) {
                // we're FinE
            }
    ...
    default Register getRegisterFromArgsOrNull(String[] args) {
        ...
        for (String s : args) {
            if (Register.fromName(s) != null)
                return Register.fromName(s); // check it straight up
        }
    ...
    default void replaceRegisterValues(String[] args) {
        ...
        int i = 0;
        for (String s : args) {
            args[i] = s.replace(getRegisterFromArgsOrNull(args).getName(), "" + getRegisterFromArgsOrNull(args).getStack());
            i++;
        }
    }
